...
class context
  ...
  context(vector_class<device> devList, async_handler asyncHandler, const device_selector &deviceSelector) {
    if (devList.empty())
    ...
    if (devList[0].is_host())
      ...
      implementation = detail::opencl_context::instance( boost::compute::context { devList[0].get_boost_compute ()} );
#endif
  }
...
public:
  ...
  /** Constructs a context object for SYCL host using an async_handler for
      ...
      default constructor.
  */
  explicit context(async_handler asyncHandler) : context( device{}, asyncHandler) {}
  ...
  */
  context() : context((async_handler)nullptr) {}
